...
enum mapistore_error mapistore_backend_folder_move_copy_messages(struct backend_context *bctx, TALLOC_CTX *mem_ctx, void *target_folder, void *source_folder, uint32_t mid_count, uint64_t *source_mids, uint64_t *target_mids, struct Binary_r **target_change_keys, uint8_t want_copy)
{
  return bctx->backend->folder.move_copy_messages(mem_ctx, target_folder, source_folder, mid_count, source_mids, target_mids, target_change_keys, want_copy);
}
