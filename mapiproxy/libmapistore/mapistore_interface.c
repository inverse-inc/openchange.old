 ...
 */
_PUBLIC_ enum mapistore_error mapistore_folder_move_copy_messages(struct mapistore_context *mstore_ctx, TALLOC_CTX *mem_ctx, uint32_t context_id,
								  void *target_folder, void *source_folder, uint32_t mid_count, uint64_t *source_mids, uint64_t *target_mids, struct Binary_r **target_change_keys, uint8_t want_copy)
{
	...
	/* Step 2. Call backend operation */
	return mapistore_backend_folder_move_copy_messages(backend_ctx, mem_ctx, target_folder, source_folder, mid_count, source_mids, target_mids, target_change_keys, want_copy);
}
