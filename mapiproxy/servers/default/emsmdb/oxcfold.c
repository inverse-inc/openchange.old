						     ...
						     uint32_t *handles, uint16_t *size)
{
	...
	if (mapistore) {
		...
		mapistore_folder_move_copy_messages(emsmdbp_ctx->mstore_ctx, mem_ctx, contextID, destination_object->backend_object, source_object->backend_object, mapi_req->u.mapi_MoveCopyMessages.count, mapi_req->u.mapi_MoveCopyMessages.message_id, targetMIDs, NULL, mapi_req->u.mapi_MoveCopyMessages.WantCopy);
