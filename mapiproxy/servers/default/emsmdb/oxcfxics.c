							  ...
							  uint32_t *handles, uint16_t *size)
{
	...
	if (mapistore) {
		/* We invoke the backend method */
          mapistore_folder_move_copy_messages(emsmdbp_ctx->mstore_ctx, mem_ctx, contextID, synccontext_object->parent_object->backend_object, source_folder_object->backend_object, 1, &sourceMID, &destMID, &change_key, false);
	}
	else {
